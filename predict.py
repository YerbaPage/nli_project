...
def evaluate(model, test_data, checkpoint, mute=False, test_bs=10):
    ...
    with torch.no_grad():
        logits, probs = model(sent_pairs, checkpoint, bs=test_bs)
    all_predict = [np.argmax(pp) for pp in probs]
    assert len(all_predict) == len(all_labels)
    print(logits)
    print(all_predict)

    # acc = len([i for i in range(len(all_labels)) if all_predict[i]
    #            == all_labels[i]])*1./len(all_labels)
    # prf = precision_recall_fscore_support(
    #     all_labels, all_predict, average=None, labels=[0, 1, 2])
...
def parse_args():
    ...
    ap.add_argument('-g', '--gpu', type=int, default=1,
                    help='run the model on gpu (1) or not (0)')
    ap.add_argument('-cp', '--checkpoint', type=int, default=0,
                    help='run the model with checkpointing (1) or not (0)')
    ap.add_argument('-tm', '--trained_model', type=str, default='default',
                    help='path to the trained model you want to test; if set as "default", it will find in output xx.state_dict, where xx is the bert-type you specified')
    ap.add_argument('-bt', '--bert_type', type=str, default='bert-large',
                    help='model you want to test; make sure this is consistent with your trained model')
    ap.add_argument('--hans', type=int, default=0,
                    help='use hans dataset (1) or not (0)')
...
if __name__ == '__main__':
    ...
    model = BertNLIModel(
        model_path=mpath, batch_size=batch_size, bert_type=bert_type)
